...
//|                                                                    |
//|  Architecture (comme discuté) :                                   |
//|    Python (regime_engine) = intelligence -> API HTTP               |
...
//|  autoriser WebRequest pour l'URL de ton API (ex: ton domaine        |
//|  Render, comme tes autres backends).                               |
//+------------------------------------------------------------------+
#property strict

input string  InpApiUrl              = "https://TON-BACKEND.onrender.com/signal"; // URL de l'API IA
input int     InpCandlesToSend       = 200;     // nombre de bougies envoyées à l'API
input int     InpTimerSeconds        = 60;      // fréquence d'interrogation de l'API

//--- Moteur de risque (mêmes règles que risk_engine.py, appliquées ici en dur)
input double  InpMaxRiskPerTradePct  = 0.5;     // % du solde risqué par trade
input double  InpMaxDailyLossPct     = 2.0;     // % perte max journalière
input double  InpMaxDrawdownPct      = 8.0;     // % drawdown max depuis le sommet d'equity
input int     InpMaxOpenPositions    = 3;
input double  InpMaxSpreadPoints     = 300;     // en points (adapter à XAUUSD)
input int     InpMaxConsecutiveLoss  = 4;
input double  InpMinImpulseProba     = 0.65;    // doit correspondre à MIN_IMPULSE_PROBA côté API
input int     InpMaxTradesPerHour    = 6;
input int     InpAtrPeriodForSL      = 14;
input double  InpAtrMultipleForSL    = 1.5;

//--- État interne (miroir de risk_engine.AccountState)
...
int OnInit()
  {
   ...
   Print("AI_Adaptive_EA initialisé. Moteur de risque local ACTIF - l'IA ne peut pas le contourner.");
...
//+------------------------------------------------------------------+
//| Boucle principale : appelée toutes les InpTimerSeconds            |
//+------------------------------------------------------------------+
void OnTimer()
  {
   ...
   if(!CallApi(json, response))
     {
      Print("Erreur d'appel API - aucune action (fail-safe : pas de trade sans signal valide).");
      return;
     }
...
//+------------------------------------------------------------------+
//| MOTEUR DE RISQUE — check_new_trade (miroir exact de risk_engine.py)|
//+------------------------------------------------------------------+
...
void TryExecuteTrade(string decision, double impulseProba)
  {
   ...
   if(decision == "achat")
     {
      ...
      tp = entry + atr * InpAtrMultipleForSL * 1.5; // R:R 1.5 par défaut, à ajuster
     }
   else
     {
   ...
   request.deviation = (int)InpMaxSpreadPoints; // slippage max toléré
   ...
   if(now.day != dayRef.day || now.mon != dayRef.mon)
     {
      g_dayStartEquity = AccountInfoDouble(ACCOUNT_EQUITY);
      g_currentDay = TimeCurrent();
      // NB: on ne reset PAS g_halted automatiquement si arrêt pour drawdown/pertes.
     }
...
//+------------------------------------------------------------------+
//| Appelle une trade fermée pour mettre à jour les pertes consécutives|
//| (à brancher sur OnTradeTransaction en usage réel)                  |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Construit le JSON des bougies à envoyer à l'API                    |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Parsing JSON minimal (sans lib externe, MQL5 natif limité)         |
//+------------------------------------------------------------------+
bool ParseSignalResponse(string json, string &regime, double &probaHausse,
                          ...
                          string &decision, bool &confidenceOk)
  {
   ...
   string confStr = ExtractJsonString(json, "confidence_ok");
