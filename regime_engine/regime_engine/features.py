...
Entrée attendue (CSV exporté MT5, ou DataFrame) :
    time, open, high, low, close, tick_volume, spread

Toutes les features sont calculées SANS look-ahead (uniquement à partir
du passé), pour éviter le biais qui fausserait le forward test.
"""
...
def load_mt5_csv(path: str) -> pd.DataFrame:
    """Charge un export MT5 (Symbol -> Export -> CSV) et normalise les colonnes."""
    df = pd.read_csv(path)
...
def session_of(ts: pd.Timestamp) -> str:
    """Session de trading approximative (heure serveur UTC à adapter à ton broker)."""
    h = ts.hour
    if 0 <= h < 7:
        return "asia"
    if 7 <= h < 12:
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Construit les features de régime + micro-structure.
    df doit contenir : time, open, high, low, close, tick_volume, spread (spread optionnel)
    """
    d = df.copy()
    ...
    # --- Volatilité réalisée (fenêtres glissantes) ---
    for w in (10, 20, 50):
        d[f"vol_{w}"] = d["log_ret"].rolling(w).std()

    # --- ATR (proxy volatilité "trading") ---
    tr = pd.concat([
        d["high"] - d["low"],
    ...
    d["atr_ratio"] = d["atr_14"] / d["atr_14"].rolling(100).mean()  # >1 = vol en expansion

    # --- Range / compression (pour détecter accumulation avant breakout) ---
    d["range_20"] = (d["high"].rolling(20).max() - d["low"].rolling(20).min())
    d["range_compression"] = d["range_20"] / d["range_20"].rolling(100).mean()

    # --- Tendance (pente de régression + position dans le range) ---
    ...
    # --- Spread (coût d'exécution) ---
    if "spread" in d.columns:
        d["spread_z"] = (d["spread"] - d["spread"].rolling(200).mean()) / d["spread"].rolling(200).std()

    # --- Sweeps de liquidité (mèche au-delà du plus haut/bas des N dernières bougies puis clôture à l'intérieur) ---
